...
*/
/* 
"C:\\Users\\THINKPAD\\AppData\\Local\\Arduino15\\packages\\arduino\\tools\\avr-gcc\\7.3.0-atmel3.6.1-arduino7/bin/avr-size" -A "C:\\Users\\THINKPAD\\AppData\\Local\\arduino\\sketches\\864EC9677872D47FA0DCF099A319D0DC/Disp8_2.ino.elf"
Sketch uses 17474 bytes (6%) of program storage space. Maximum is 253952 bytes.
...
void cargarDesdeEEPROM() {
  ...
  if (flag != EE_FLAG_INIT) {
    for (int i = 0; i < 5; i++) EEPROM.write(EE_ADDR_MACHINEID + i, DEFAULT_MACHINE_ID[i]);
    ...
    for (int i = 0; i < 5; i++) MACHINE_ID[i] = EEPROM.read(EE_ADDR_MACHINEID + i);
    MACHINE_ID[5] = '\0';
...
void setup() {
  ...
  sendFrameOutBoth(MACHINE_ID, "BOOT", 0, 82);
  Beep_On();
}
